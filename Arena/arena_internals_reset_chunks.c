...
void *_arena_reorder_reset_chunk(t_dynamic_array *reset_chunks, void *reset_chunk_ptr, size_t size_bytes, size_t size_available)
{
...
void *_arena_use_reset_chunk(t_arena *arena_list, size_t size_bytes)
{
	...
	while (arena_list)
	{
		...
		{
			reset_chunk_ptr = *(void **)((char*)arena_list->reset_chunks->data + i * arena_list->reset_chunks->data_size);
			data_ptr_after = _arena_lookup_next_ptr(arena_list, reset_chunk_ptr);
			size_available = data_ptr_after - reset_chunk_ptr;
			if (size_available >= size_bytes)
				return (_arena_reorder_reset_chunk(arena_list->reset_chunks, reset_chunk_ptr, size_bytes, size_available));
			i++;
		}
